package com.rl.util;

public class Keys {
    public static String field_work="https://rlwork.in/api_gfhjfyRETfkmghTYudgnm/field_work_sfrfregRGdfghtRTr/";

    public static class URL{
        public static String register = field_work + "register.php";
        public static String login = field_work + "login.php";
        public static String forgotpassword = field_work + "forgotpassword.php";
        public static String getprofile = field_work + "get_profile.php";
        public static String add_request = field_work + "add_request.php";
        public static String add_bank_details = field_work + "add_bank_details.php";
        public static String kyc_details = field_work + "kyc_details.php";
        public static String show_quotation = field_work + "show_quotation.php";
        public static String customer_call_request = field_work + "call_request.php";

        public static String show_bill = field_work + "show_bills.php";
        public static String change_password = field_work + "change_password.php";
    }

}
